...
export async function run() {
  try {
    ...
    const resp = await gh_client.request(`GET /repos/${owner}/${repo}/languages`);

    let languages = resp.data;

    core.debug(`Got ${JSON.stringify(resp.data)}`);

    const total = Object.values(languages).reduce((accumulator, current) =>
      accumulator + current,
      0
    );
    ...
    if (allowed_languages_str.trim().length > 0) {
      let allowed = allowed_languages_str.split(',')
        .map((x) =>
          x.trim().toLowerCase()
        );
